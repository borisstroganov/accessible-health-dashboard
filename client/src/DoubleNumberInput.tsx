...
function DoubleNumberInput({ onClick, title, subTitle, labelOne, labelTwo }: DoubleNumberInputProps) {
    ...
    return (
        <>
            {toggleModal && <Modal onClick={() => onClick(formValueOne, formValueTwo)} onCancel={() => setToggleModal(false)} headerText="Confirmation" bodyText={`Are you sure you want to submit - ${formValueOne} and ${formValueTwo}?`} buttonText="Submit" buttonTextColor="limegreen" />}
            <div className="form-wrapper">
                ...
                <div className="double-input-wrapper">
                    <div className="input-wrapper">
                        ...
                        <input type="number" className="number-input" min="0" max="999" value={formValueOne} onChange={(e) => parseInt(e.target.value) < 1000 || e.target.value == "" ? setFormValueOne(parseInt(e.target.value)) : ""} />
                        <h4>{labelOne}</h4>
                    ...
                    <div className="input-wrapper">
                        ...
                        <input type="number" className="number-input" min="0" max="999" value={formValueTwo} onChange={(e) => parseInt(e.target.value) < 1000 || e.target.value == "" ? setFormValueTwo(parseInt(e.target.value)) : ""} />
                        <h4>{labelTwo}</h4>
                        <button type="button" className="plus" onClick={handlePlusTwo}><AiOutlinePlusCircle /></button>
                    </div>
                </div>
                <button type="button" className="submit" onClick={() => setToggleModal(true)} disabled={!formValueOne || !formValueTwo}>Submit</button>
            </div>
        </>
    )
}
