    ...
    return (
        <>
            {toggleModal && <Modal onClick={() => onClick(formValue)} onCancel={() => setToggleModal(false)} headerText="Confirmation"
                bodyText={`Are you sure you want to submit - ${formValue}?`} buttonText="Submit" buttonTextColor="limegreen" />}
            <div className="form-wrapper">
                <h3>{title}</h3>
                <div className="input-wrapper">
                    <button type="button" className="minus" onClick={handleMinus}><AiOutlineMinusCircle /></button>
                    <input type="number" className="number-input" min="0" max="999" value={formValue} onChange={(e) =>
                        parseInt(e.target.value) < 1000 || e.target.value == "" ? setFormValue(parseInt(e.target.value)) : ""} />
                    <h4>{label}</h4>
